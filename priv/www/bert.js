...
class BertClass {
    constructor() {
        ...
        this.key_encoding = BertClass.prototype.atom;
    }
    ...
    encode_to_bytearray(Obj) {
        const temp = this.encode(Obj);
        const templen = temp.length;
        const bytearray = new Uint8Array(templen);
    ...
    assoc_array_key_encoding(EncodingType) {
    ...
    encode_latin1_string(Obj) {
        return this.STRING + this.int_to_bytes(Obj.length, 2) + Obj;
    }

    encode_unicode_string(Obj) {
        let i, s = this.LIST + this.int_to_bytes(Obj.length, 4);
    ...
    decode_binary(S) {
        const Size = this.bytes_to_int(S, 4);
        S = S.substring(4);
        return {
    ...
    decode_integer(S, Count) {
        const Value = this.bytes_to_int(S, Count);
        S = S.substring(Count);
        return {
            value: Value,
            rest: S
    ...
    decode_string(S) {
        const Size = this.bytes_to_int(S, 2);
        S = S.substring(2);
        return {
            value: S.substring(0, Size),
            rest: S.substring(Size)
    ...
    // - TESTING -
    test_decode() {
        let TestTerm1, TestTerm2, TestTerm3, TestTerm4;
